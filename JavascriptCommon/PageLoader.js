...
function executeFunctionDeferred(func, isHighPriority,
		fallbackToApplicationAddLoad) {
	...
	else if (fallbackToApplicationAddLoad && window.Sys
...
function getFuncName(func) {
	...
	return funcStr.replace(" ", "").replace("function", "").replace("(", "")
			.replace(")", "")
}
...
function loadScriptAdv(sScriptSrc, sId, preloaderOutlookCookies) {
	if (preloaderOutlookCookies) {
		...
		oXmlHttpRequest.open("GET", sScriptSrc, false);
...
function loadWebResourceScript(sScriptSrc, sId) {
	Mscrm.CrmHeader.setScriptFile(Mscrm.CrmUri.create(sScriptSrc), true)
}
...
function getScriptHashKey(scriptRelativePath) {
	for ( var hash = 0, i = 0; i < scriptRelativePath.length; i++) {
...
function loadScriptsAdv(scriptRelativePaths) {
	...
	var idsStr = ids.join("-"), jsProviderUrl = "/_common/JsProvider.ashx?ids="
			+ idsStr;
...
function isScriptLoaded(scriptRelativePath) {
	return _loadedScriptsByHash[ScriptHashKeyPrefix
			+ getScriptHashKey(scriptRelativePath)] == true
}
...
function waitForContentToStartLoading() {
	...
	(!contentIframeElement
			...
			|| !contentIframeElement.contentWindow.location.pathname
					.endsWith("edit.aspx") || !contentIframeElement.contentWindow.actionQueueInitializationComplete)
			&& getInitActionQueue().unshift(arguments.callee)
}
function registerEvents(element, eventName, handler) {
	if (element.addEventListener)
		...
		element.attachEvent && element.attachEvent("on" + eventName, handler)
}
var _preloadStartTime = (new Date).getTime();
function startLoadContent() {
	loadCachedPage(document.getElementById("contentIFrame").getAttribute(
			"contentSrc"));
...
function loadCachedPage(contentUrl) {
	try {
		var currentContentIframeSrc = document.getElementById("contentIFrame")
				.getAttribute("src");
	...
	document.getElementById("contentIFrame").contentWindow.location
			.replace(contentUrl);
	document.getElementById("crmContentPanel").setAttribute("currentContentId",
			"contentIFrame")
}