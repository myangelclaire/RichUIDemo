...
Mscrm.RelationCheckBoxBehavior.prototype = {
	...
	get_primaryObjectTypeCode : function() {
		return parseInt(this.get_element().getAttribute("PrimaryOTC"), 10)
	},
	...
	get_associatedObjectTypeCode : function() {
		return parseInt(this.get_element().getAttribute("AssociatedOTC"), 10)
	},
	...
	get_isDirty : function() {
		return this.$0_4 !== parseInt(this.get_dataValue().toString(), 10)
	},
	...
	get_dataXml : function() {
		...
		if (this.get_elementValue() === "1") {
			$v_0
					.append('action="Create" ><dataxml><relationshiprolemap><primaryobjecttypecode>');
			$v_0
					.append(String
							.format(
									"{0}</primaryobjecttypecode><associateobjecttypecode>{1}</associateobjecttypecode></relationshiprolemap></dataxml>",
									this.get_primaryObjectTypeCode(), this
											.get_associatedObjectTypeCode()))
		} else
			$v_0.append(String.format('id="{0}" action="Delete" >',
					CrmEncodeDecode
							.CrmXmlAttributeEncode(this.get_element().id)));
		$v_0.append("</item>");
		return $v_0.toString()
	},
	...
	toggle : function() {
		!this.get_disabled()
				&& this
						.set_elementValue(parseInt(this.get_elementValue(), 10) === 1 ? "0"
								: "1")
	}
};
Mscrm.RelationCheckBoxBehavior.registerClass("Mscrm.RelationCheckBoxBehavior",
